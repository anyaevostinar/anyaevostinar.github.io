...
As always you should test your code as you implement each method. For each required method in `WordCountTree` **you must have an example of it working** in the `main()` method of `WordCountTree.java`. At minimum these examples should call the method, those double checking the results would be a good additional thing to do. I highly recommend you create a smaller text file to use in your testing.
...
which would generate the word cloud based on the 40 most common non-stopwords in `WutheringHeights.txt` and output the cloud HTML to the terminal. (If the text contains fewer than 40 non-stopwords, then the cloud will just use all the words, and if some words are tied, any method of tie-breaking is fine.)
...
|decrementCount method implemented correctly | 2 |
|decrementCount method example provided|1|
