...
* DNA is actually double-stranded where each A must pair with a T in the other strand and each C must pair with a G. In computer science, this is called a Multi-Linked List. Update your model to have two strand and each has a connection to its pair in the other list. (You'll need to get creative in how you alter `Node` and probably will want to stop inheriting from `LinkedUnorderedList` at this point, though you could also have a new `DNA` class that holds two `DNAStrand`s and maintains the connection between them too.)